<h1 align="center">What You Need, You Get: Using Vim as an IDE</h1>
...
##【公告】
----
* **For discussion and suggestions**, see http://www.v2ex.com/t/138696 (discussion mainly in Chinese)
...
Before we start, I assume: 0) you know the very basics of vim, such as opening, editing, and saving files, and switching between normal and insert mode; 1) you want to turn your vim into a C/C++ IDE, rather than any other language.

You can easily find 128+ vim's advantages on Google, but for me, it has only two: 0) What you think, you get, since it makes editing as fast as thinking; 1) What you need,  you get, because every imaginable functionality can be achieved through vim script as a plugin. To get the first ability, you can study two books: *Practical Vim: Edit Text at the Speed of Thought* and *vim user manual*. To get the second ability, read this guide. -。-#
...
<div align="right">
--yangyangwithgnu@yeah.net<br />
</div><br />  

Back to vim, let's talk about how the vim editor supports code editing directly or indirectly. In the *vim user manual*, over half of examples are about coding efficiently. It's pretty obvious then that vim is made for programmers. Even though the core editor itself does not provide lots of the essential coding functionalities, its robust plugin ecosystem enables you to get whatever functionalities you need to code. That is the meaning of *what you need, you get*.

I am a believer of clear objectives. This guide will first list what qualities or functionalities a program needs in order to qualify for an excellent C/C++ IDE and then explore how to use vim and its plugins to achieve them. In the end, your programming environment should at least look like this:
...
(In pure text environment)
</div>

<h2 name="0">0 Vim Must-knows</h2>
Before officially starting, let me first introduce some essential knowledge about vim. It is not about how to configure our vim environment, but it is crucial for understanding the following configurations.

<h3 name="0.1">0.1 The .vimrc File</h3>
.vimrc is a configuration file that dictates the behaviors of the vim editor. It is located at ~/.vimrc. This .vimrc can change vim's appearance, font, behavior, keyboard shortcut, and plugin options. By fine-tuning this file, you can make vim the perfect code editor just for yourself.

The reason why it's so hard to get started with vim is the lack of "defaults," or pre-configured functionalities. You cannot even find the .vimrc file after initial install. Without any configurations, vim is indeed ugly and hard to use. For any task, from code editing or text editing, I think the following basic configurations is necessary.

**The leader key.** All kinds of vim plugins often refer to the leader key as \<leader>. This leader key is introduced to resolve conflicts between built-in vim shortcuts and various others introduced by all sorts of plugins. With leader, the shortcut `r` can be configured as `<leader> r` or even `<leader><leader>r`. This leader key is probably the most used key besides `Esc`. Therefore, choosing an easy-to-reach leader key is critical for improving efficiency. I would recommend `;` as the leader key since it's right under you right pinkie, enabling you to reach it without looking or moving your hand.

```
" Configure the leader key, <Leader>
let mapleader=";"
```
As \<leader> is born for shortcuts, let's then talk about a simple rule to follow when configuring shortcuts: different shortcuts should not include the same keystroke sequence. For example, we configure`<leader> e` to do task A and `<leader> eb` for some task B. This way, when we enter `<leader>e`, vim will not immediately execute task A even if we want it to. Vim would rather continue waiting for a potential `b` for some time in order to differentiate between task A and B. This will obviously slow down your task A. If we instead configure task A and task B as respectively `<leader>ea` and `<leader>eb`, we won't have this problem.

**File type detection.** File type detection allows you to load different plugins for different file types (e.g. syntax highlighting plugins for C++ is different from those for python).

```
" Turn on file type detection
filetype on
" Load different plugins based on different detected file types
filetype plugin on
```

**Keyboard shortcuts.** Configure most-used vim built-in actions as keyboard shortcuts to improve efficiency.

```
" Navigating to line beginning and ending
nmap LB 0
nmap LE $
" Yanking selected text to system clipboard
vnoremap <Leader>y "+y
" Pasting from system clipboard to vim
nmap <Leader>p "+p
" Closing current window
nmap <Leader>q :q<CR>
" Saving in current window
nmap <Leader>w :w<CR>
" Saving content in all windows and exiting vim
...
**Config hot-reload.** In this guide, we will frequently change .vimrc. The usual way to reload configuration is to save .vimrc and then restart vim, which would be a disaster if you want follow along with this guide. Add the following config to automatically reload config when saving .vimrc.

```
" Config hot-reload
autocmd BufWritePost $MYVIMRC source $MYVIMRC
```
With this, we can change our color scheme at any time:
...
**Others.** Search, vim command auto-completion, etc.:

```
" Turn on realtime search
set incsearch
" Turn off case sensitivity
set ignorecase
" Turn off compatible mode
set nocompatible
" Turn on auto-completion for vim built-in commands
set wildmenu
```

The above four kinds of configurations influence not only vim itself, but also various plugins. Thus, many plugins require configurations for both the plugins themselves and the core vim editor to function properly. The following guide assumes that you have configured vim as above and will only discuss config for plugins. If you cannot get some plugins to work as this guide shows, it is highly probably a result of missing config for the core vim editor. In those cases, come back to this section and copy the above config to your .vimrc file. The complete .vimrc is attached at the end with every config commented. Besides, vim may show error messages because you have not installed and configured all other plugins that a plugin depends on. Don't panic and keep going. Those errors will disappear when you are finished with all plugins.

<h3 name="0.2">0.2 The .vim/ Directory</h3>

The .vim/ directory is where we keep all our plugins. Vim has a scripting language of its own: vimscript. With vimscript, we can interact with vim to extend its functionalities. A set of vimscript that implements some extra functionalities is a plugin. Vimscript and plugins built with it are how vim achieves its versatility. Other than vimscript, vim also supports other mainstream scripting languages such as, perl, python, lua, and ruby, given that vim is compiled with respective options like `--enable-perlinterp`、`--enable-pythoninterp`、`--enable-luainterp`、`--enable-rubyinterp`. [Vim online][www.vim.org] and [GitHub][www.github.com] hosts a rich variety of plugin resources. Whenever you want some functionality that vim does not directly support, you can go to those resource to hunt for plugins that support it.

Vim plugins are now either *.vim or *.vba. The former is a usually just text files, usually packaged with `someplugin.vim` (the plugin script) and `someplugin.txt` (plugin help). To install those plugins, we only need to put `someplugin.vim` into `~/.vim/plugin` and `someplugin.txt` to `~/.vim/doc`. After restarting vim, the plugin itself should start working. To get its help files, we need to call `:helptags ~/.vim/doc/`. Then, we can check help information for the plugin using `:h someplugin`. The installation process for this kind of plugins can be annoying. That's why people later came up with *.vba plugins. To install those, we only need to run the following commands:
...
:q
```
Both kinds of plugins can be difficult to uninstall and update, so some people made plugin managers like vundle.
...
<h2 name="2">2 Plugin management</h2>
The goal of this guide is to talk about how to turn vim into an ideal C/C++ IDE. Then efficiently managing plugins is the first problem we need to solve.

Vim originally hopes to use subdirectories in the `.vim/` directory to manage all plugins. For example, we might use subdirectory `doc/` to store documentations and help files and subdirectory `plugin/` to store plugin scripts. Vim plugin packages often have those two directories as well (or even more). Users need to copy those corresponding directories to `.vim/`'s subdirectories to install a plugin. This paradigm works fine in many senarios. But for us, who will use plugins intensively, `.vim/` will eventually turn into complete chaos. More specifically, this paradigm has the following problems:

* Plugin name conflict. The help files for all plugins are stored in `doc/` and all scripts are put in `plugin/`. This way, plugins with the same name will cause comflicts.
* It's a pain to uninstall plugins. Users need to figure out what files under `doc/` and `plugin/` belong to the plugin. This might lead to mis-removal or incomplete removal.

We want every plugin to have an independent subdirectory under `.vim/`. This way when we need to update or uninstall plugins, we only need to change files in one directory. Besides, we want to be able to list all plugins in some sort of configuration files and use some automated process to install/update/upgrade all plugins. vundle (https://github.com/VundleVim/Vundle.vim) is born for this. It makes plugin management clear and clever.

vundle will take over all directories under `.vim/` so we need to first purge that directory. Then we can install vundle with:

```
git clone https://github.com/VundleVim/Vundle.vim.git ~/.vim/bundle/Vundle.vim
```
Then we need to add config info in `.vimrc`:

```
" vundle environment config
filetype off
set rtp+=~/.vim/bundle/Vundle.vim
" all plugins managed by vundle has to be located between vundle#begin() and vundle#end()
call vundle#begin()
...
```
In the list, every line like this

```
Plugin 'dyng/ctrlsf.vim'
```

corresponds to a plugin (this is similar to how Go takes care of different modules). If later we want new plugins, we only need to add it to the list. vundle supports all plugins hosted on [GitHub][https://github.com] and all plugins on the vim website is mirrored on https://github.com/vim-scripts. Therefore, vundle can manage most mainstream plugins on GitHub and on the vim website. Take ctrlsf as an example. It has config path: `dyng/ctrlsf.vim`. With this, vundle can easily construct the full url: https://github.com/dyng/ctrlsf.vim.git. Then vundle will use git to download and install the plugin.

After this, whenever we need to install a plugin, we only need to find its GitHub address, add its info between `call vundle#begin()` and `call vundle#end()`, and finally run this command in vim:

```
:PluginInstall
```
This way vundle can automatically install this plugin and its documentation. For example, we are adding four plugins to `.vimrc`:
...
Plugin 'octol/vim-cpp-enhanced-highlight'
```
Currently, `.vim/` is empty. When I execute `:PluginInstall`, the entire installation process has started.
<div align="center">
...
（vundle installing plugins in batch）
</div>
If we want to uninstall a plugin, we need to comment out or remove its configuration and then run:

```
:PluginClean
```
This way we have removed the plugin. Since plugins update quite frequently, we should check for updates approximately once a month. To do that, we will run:

```
:PluginUpdate
```

We need to be careful about the source of our plugins. GitHub usually has many plugins with similar names. For example, if we want an indexer plugin, we have https://github.com/vim-scripts/indexer.tar.gz, https://github.com/everzet/vim-indexer, https://github.com/shemerey/vim-indexer. Then which one should we use? According to my past experience, if we know what plugin we are looking for, we should first check the personal website of the plugin author. He or she usually posts the GitHub address to his or her plugin. If not, we should look on www.vim.org, which also usually has GitHub links. If not, we will then search GitHub with keywords. The one with more stars is most probably what we are looking for. To save your time, I have listed the address of all plugins I will be using. I will not be talking about how to install them in normal cases. 
